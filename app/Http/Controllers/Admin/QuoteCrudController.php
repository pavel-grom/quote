...
class QuoteCrudController extends CrudController
{
    ...
    protected function setupListOperation()
    {
        ...
        CRUD::column('episode')->type('relationship');
        CRUD::column('character')->type('relationship');
    ...
    protected function setupCreateOperation()
    {
        ...
        $episodes = Episode::with('characters:id,name')
            ->select(['id', 'title'])
            ->get();
        ...
        foreach ($episodes as $episode) {
            foreach ($episode->characters as $character) {
                $episodesCharacters["{$character->id}_{$episode->id}"] = "{$episode->title} / {$character->name}";
            }
        }
        ...
        CRUD::addField([
            ...
            'label' => 'Episode / Character',
            'type' => 'select2_from_array',
            'options' => $episodesCharacters,
        ]);
